...
На всех внутренних страницах использовать общий `PageHeader`: кнопка «Назад» слева, меню справа. Только главная передаёт `backHref={false}`; остальные страницы задают родительский маршрут для прямого открытия. Дополнительные действия передаются через `actions`, отдельные панели — через `children`. Модальные формы используют `EntityFormHeader` с отменой и сохранением.
...
- Для выбора периода на главной и в транзакциях использовать общий `PeriodControl`: стрелки смещения, единые подписи и календарь произвольного периода.
