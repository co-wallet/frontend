...
describe('active filter summary', () => {
  ...
  it('shows one value per type with independent overflow counts', () => {
    ...
    expect(markup).toContain('role="group" aria-label="Счета"')
    expect(markup).toContain('role="group" aria-label="Категории"')
    expect(markup).toContain('role="group" aria-label="Теги"')
    expect(markup.match(/aria-label="Убрать фильтр /g)).toHaveLength(3)
    ...
    expect(markup).toContain('Счета: ещё 2. Открыть фильтры')
    expect(markup).toContain('Категории: ещё 1. Открыть фильтры')
    expect(markup).toContain('Теги: ещё 1. Открыть фильтры')
    expect(markup).toContain('Сбросить все')
  })

  it('omits empty types and overflow for a single selected value', () => {
    const markup = renderPage('/transactions?tag_ids=t1')
    expect(markup).toContain('role="group" aria-label="Теги"')
    expect(markup).not.toContain('role="group" aria-label="Счета"')
    expect(markup).not.toContain('role="group" aria-label="Категории"')
    expect(markup).not.toContain('Ещё')
    expect(markup).toContain('aria-label="Убрать фильтр по тегу путешествия"')
    expect(markup).toContain('title="путешествия"')
  })
  ...
  it('keeps unknown selections visible and counted while dictionaries are unavailable', () => {
    ...
    expect(markup).toContain('aria-label="Убрать фильтр по счёту Счёт"')
    expect(markup).toContain('aria-label="Убрать фильтр по категории Категория"')
    expect(markup).toContain('#Тег')
    expect(markup).toContain('Счета: ещё 1. Открыть фильтры')
  })
})
