...
function explain(p: ImportPreview, d: Diagnostic) {
  ...
  switch (d.code) {
    ...
    case 'target_account_members': return { title: `${name}: проверьте участников и доли`, action: d.message + ' Исправьте настройки счёта выше и обновите предпросмотр.' }
    case 'target_flags': return { title: 'Счета станут активными; доступ выбирается отдельно', action: 'Флаг включения в общий баланс Monefy не переносится. Выберите тип каждого счёта выше: он определяет, в какой группе средств счёт отображается.' }
...
export function MonefyImportDiagnostics({ state, controller }: { state: ImportState; controller: MonefyImport }) {
  ...
  return <IonCard>
    ...
    <IonCardContent>
      {missing.length > 0 && <p>Осталось выбрать тип счетов: {missing.length}. Заполните поля в разделе «Счета и распределение» выше — файл для этого менять не нужно.</p>}
