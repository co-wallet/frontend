...
export function AccountMembersFields({ value, onChange, ownerUsername, error, disabled = false }: {
  value: MemberDraft[]; onChange: (members: MemberDraft[]) => void; ownerUsername: string; error?: string; disabled?: boolean
}) {
    ...
    {value.map((member, index) => <div key={index}>
      <IonItem>
        {member.username === ownerUsername ? <IonNote>{ownerUsername} · Владелец</IonNote>
          : <EntityFormSelect disabled={disabled} label={`Участник ${index + 1}`} value={member.username} onIonChange={(e) => onChange(value.map((m, i) => i === index ? { ...m, username: e.detail.value } : m))}>
      ...
      <IonItem>
        <IonInput disabled={disabled} label={`Доля ${member.username || index + 1}`} labelPlacement="stacked" inputMode="decimal" value={member.share}
          onIonInput={(e) => onChange(value.map((m, i) => i === index ? { ...m, share: filterDecimalInput(e.detail.value ?? '') } : m))} />
        {member.username !== ownerUsername && <IonButton disabled={disabled} fill="clear" color="danger" aria-label={`Убрать участника ${index + 1}`} onClick={() => onChange(value.filter((_, i) => i !== index))}>Убрать</IonButton>}
      </IonItem>
    </div>)}
    <IonButton fill="clear" disabled={disabled || isLoading || isError || value.some((m) => !m.username) || !users.some((u) => !value.some((m) => m.username === u.username))}
