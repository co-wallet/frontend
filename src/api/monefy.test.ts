...
describe('Monefy API contract', () => {
  ...
  it('uses immutable preview IDs for options and confirmation', async () => {
    ...
    expect(client.post).toHaveBeenCalledWith('/imports/monefy/old/options', { account_access: {}, account_kinds: { a: 'deposit' }, category_icons: {}, account_icons: {} })
  ...
  it('sends account and category appearance in the same options snapshot', async () => {
    ...
    expect(client.post).toHaveBeenCalledWith('/imports/monefy/p/options', { account_access: {}, account_kinds: { a: 'spending' }, category_icons: { c: 'preset:cafe|red|none' }, account_icons: { a: 'preset:cash|green|green' } })
  })
